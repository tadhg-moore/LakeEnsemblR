...
#' @param btm_depth numeric; Depth to extract the bottom temperature from, must be positive. If none provided uses the max depth in the observed file. Defaults to NULL
...
export_init_cond <- function(model = c('GOTM', 'GLM', 'Simstrat', 'FLake'), wtemp_file, date, month = NULL, ndeps = 2, btm_depth = NULL, print = TRUE, folder = '.'){
  ...
  if(!is.null(month)){
    ...
    if(is.null(btm_depth)){
      btm = max(sub2[,1], na.rm = TRUE)
    }else{
      btm = btm_depth
    }
    top = min(sub2[,1], na.rm = TRUE)
  ...
  if('GLM' %in% model){
    ...
    glm_deps <- deps[order(deps)]
    glm_tmp <- tmp[order(deps)]
    # Input to nml file
  ...
  if('Simstrat' %in% model){
    sim_deps <- -deps[order(deps)]
    sim_tmp <- tmp[order(deps)]
